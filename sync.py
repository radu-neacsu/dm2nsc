...
MINUTES_BETWEEN_SLOW_CARB_ENTRY = 15
...
def get_entries(login):
    ...
    fromDate = arrow.now().shift(minutes=-5)
...
def convert_nightscout(entries, start_time=None):
    ...
    for entry in entries:
        time = arrow.get(int(entry["entry_time"]) / 1000).to(entry["timezone"]).shift(minutes=1)
        notes = entry["notes"]
        notes.replace("[Nightscout]", "")
        if start_time and start_time >= time:
            continue
        author = NS_AUTHOR
        notes = json.dumps({'author': author, 'entry_id': entry['entry_id'], 'notes': notes})
        ...
        out.append(dat)
        add_slow_carbs_entries(entry, out)
...
def get_last_nightscout():
    last = requests.get(
        NS_URL + 'api/v1/treatments?count=1&find[enteredBy]=' + NS_AUTHOR)  # +urllib.parse.quote(NS_AUTHOR))
...
def add_slow_carbs_entries(entry, out=[]):
    if entry['extended_bolus'] > 0 and entry['extended_bolus_duration'] > 0:
        number_of_ecarb_entries = int(entry['extended_bolus_duration'] / MINUTES_BETWEEN_SLOW_CARB_ENTRY)
        total_eCarb = entry['extended_bolus'] * entry['carb_ratio_factor']
        eCarb = total_eCarb / number_of_ecarb_entries
        time = arrow.get(int(entry["entry_time"]) / 1000).to(entry["timezone"])
        i = 0
        while number_of_ecarb_entries > i:
            i += 1
            notes = json.dumps({'entry_id': entry['entry_id'], 'eCarb_no': i, 'eCarbTotal': number_of_ecarb_entries})
            author = NS_AUTHOR+":eCarb_"+str(i)
            time = time.shift(minutes=MINUTES_BETWEEN_SLOW_CARB_ENTRY)
            dat = {
                "eventType": "Meal Bolus",
                "created_at": time.format(),
                "carbs": eCarb,
